...
logger.info("Create user response: %s", create_user_response.model_dump(by_alias=True))

user_id = create_user_response.user.id

# 2) Open credit card account
open_credit_card_account_response = accounts_client.open_credit_card_account(user_id)
logger.info("Open credit card account response: %s", open_credit_card_account_response)
